...
const analyzeWithAI = async (content) => {
  try {
    ...
    // Normalize different possible SDK shapes and await text if needed
    let text = '';
    if (result?.response) {
      const resp = result.response;
      text = typeof resp === 'string' ? resp : (typeof resp.text === 'function' ? await resp.text() : String(resp));
    } else if (result?.output && Array.isArray(result.output)) {
      // common generative output shape
